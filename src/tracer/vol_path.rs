...
impl Trace for VolPath {
    ...
    fn trace(&self, scene: &Scene, mut sampler: Sampler, ray: R) -> Color {
        ...
        |(mut li, mut tp, ray, its, spec), depth| its.map(|its| {
            let ray = ray.clipped(its.t);
            let (tr, it) = ray.sample_tr(&mut sampler);
            tp *= tr;
            if tp == Color::BLACK { return Either::L(li) }

            let its = it.unwrap_or(its);
            let (ray, its, spec) = match its.phase {
                None => {
                    if !its.has_bsdf() {
                        let ray = its.ray_for(ray.d, ray.t - its.t);
                        return Either::R((li, tp, ray, scene.intersect(ray), spec))
                    }

                    let (ld, res) = Direct::li(scene, &mut sampler, &its, &ray, true);
