...
impl N {
    #[inline(always)] pub fn v(v: V) -> N { N(v.unit()) }
    #[inline(always)] pub fn a3(v: F3) -> N { N::v(V(v)) }
}
...
impl Mul<N> for T { type Output = N;
    #[inline(always)] fn mul(self, n: N) -> N { N::v(self.inv().t() * *n) }
}

impl Div<N> for T { type Output = N;
    #[inline(always)] fn div(self, n: N) -> N { N::v(self.inv().t() / *n) }
}
