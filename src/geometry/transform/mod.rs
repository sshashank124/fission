...
impl Inv for TransformPair3 { type Output = Self;
    #[inline(always)] fn inv(self) -> Self { Self::new(self.i, self.f) }
}

impl Mul for TransformPair3 { type Output = Self;
...
where
    B: Copy + Mul<F, Output=B>
     + Add<Output=B>
     + Add<F, Output=B>
{
    type Output = A3<B>;
    #[inline(always)] fn mul(self, t: A3<B>) -> A3<B> { self.f * t }
}
...
{
    type Output = A3<B>;
    #[inline(always)] fn div(self, v: A3<B>) -> A3<B> { self.i * v }
}
